  ...
  return (
    <div className="hidden md:flex w-[280px] bg-bg-1 border-l border-border-default flex-col shrink-0">
      <div className="flex border-b border-border-default shrink-0">
        <button onClick={() => setTab("properties")} className={`flex-1 py-2 text-[11px] font-medium transition-colors ${activeTab === "properties" ? "text-text-0 border-b border-green" : "text-text-3 hover:text-text-1"}`}>
          Properties
        </button>
        <button onClick={() => setTab("design")} className={`flex-1 py-2 text-[11px] font-medium transition-colors ${activeTab === "design" ? "text-text-0 border-b border-green" : "text-text-3 hover:text-text-1"}`}>
          Design
        </button>
      </div>
