...
export function VersionHistory() {
  ...
  return (
    <div ref={panelRef} className={`absolute top-0 right-0 bottom-0 w-80 bg-bg-1 border-l border-border-default z-50 flex flex-col transition-transform duration-250 ease-in-out ${historyOpen ? "translate-x-0" : "translate-x-full"}`}>
      <div className="px-4 py-3.5 border-b border-border-default flex items-center justify-between">
        ...
        <button onClick={toggleHistory} className="w-7 h-7 rounded flex items-center justify-center text-text-3 hover:text-text-0 hover:bg-bg-3 transition-colors">
          <X size={14} />
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-2">
        <div className="p-3 rounded-lg bg-green-glow mb-0.5">
          <div className="flex items-center gap-1.5 text-[11px] text-text-2 mb-1">
            <span>Current</span>
            <span className="text-[9px] px-1.5 py-0.5 rounded-full font-semibold bg-green-glow text-green">latest</span>
          </div>
          <div className="text-[12.5px] text-text-1">Current state</div>
        </div>

        {entries.map((entry, i) => (
          <div key={i} onClick={() => { for (let n = 0; n <= i; n++) useConfigStore.getState().undo() }} className="p-3 rounded-lg cursor-pointer transition-colors mb-0.5 hover:bg-bg-3 group">
            <div className="flex items-center gap-1.5 text-[11px] text-text-2 mb-1">
              <span>{timeAgo(entry.timestamp)}</span>
              <span className="text-[9px] px-1.5 py-0.5 rounded-full font-semibold bg-status-blue/10 text-status-blue">manual</span>
            </div>
        ...
        {entries.length === 0 && <div className="p-4 text-center text-[11px] text-text-3">No history yet. Make some changes to see history.</div>}
      </div>
    </div>
  )
}
