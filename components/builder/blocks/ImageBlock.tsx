...
export function ImageBlock({ block }: { block: BlockConfig }) {
    ...
    return (
      <div className="px-6 py-12 @lg:px-16 @lg:py-16">
        <div className="grid grid-cols-1 @lg:grid-cols-2 gap-8 @lg:gap-12 items-center">
          <div className={`reveal-scale reveal-d1 ${imageSide === "right" ? "@lg:order-2" : ""}`}>{imgEl}</div>
  ...
  if (variant === "grid") {
    const gridImages = images.length > 0 ? images.slice(0, 4) : [{ src: undefined, alt: "1" }, { src: undefined, alt: "2" }, { src: undefined, alt: "3" }, { src: undefined, alt: "4" }]
    return (
      <div className="px-6 py-12 @lg:px-16 @lg:py-16">
