      ...
      <div className="flex flex-col @lg:flex-row items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-4">
          {(props.links || []).map((link, i) => (
            <span key={i} className="text-[12px] text-text-3 hover:text-text-1 transition-colors cursor-pointer">
              {link}
            </span>
          ))}
        </div>
    ...
    <footer className="px-6 @md:px-10 py-12 border-t border-border-subtle">
          ...
          <div key={i}>
            ...
            <ul className="space-y-2">
              {col.links.map((link, j) => (
                <li key={j}>
                  <span className="text-[12.5px] text-text-3 hover:text-text-1 transition-colors cursor-pointer">{link}</span>
                </li>
              ))}
            </ul>
          </div>
      ...
      <div className="pt-6 border-t border-border-subtle flex flex-col @lg:flex-row items-center justify-between gap-3">
        ...
        <div className="flex gap-4">
          {(props.links || []).map((link, i) => (
            <span key={i} className="text-[11px] text-text-3 hover:text-text-1 transition-colors cursor-pointer">
              {link}
            </span>
          ))}
        </div>
  ...
  return (
    <footer className="px-6 @md:px-10 py-6">
      <div className="flex items-center justify-center gap-1.5 text-[11px] text-text-3">
        ...
        {links.length > 0 && <span className="mx-1">|</span>}
        {links.map((link, i) => (
          <span key={i}>
