...
const defaultTestimonials: Testimonial[] = [
  { name: "Sarah Chen", role: "CEO at TechCorp", quote: "This builder completely changed how we ship landing pages. The JSON-first approach is genius.", rating: 5 },
...
function StarRating({ rating }: { rating: number }) {
  return (
    <div className="flex gap-0.5">
      {[1, 2, 3, 4, 5].map((i) => (
        <Star key={i} size={12} className={i <= rating ? "text-status-yellow fill-status-yellow" : "text-text-3"} />
      ))}
    </div>
  )
}
  ...
  return (
    <section className="px-6 @md:px-10 py-16 @md:py-20">
      <div className="reveal-fade-up reveal-d1 text-center mb-10">
        <h2 className="text-2xl @md:text-3xl font-bold tracking-tight mb-2">{props.title || "What people say"}</h2>
        {props.subtitle && <p className="text-text-2 text-sm max-w-lg mx-auto">{props.subtitle}</p>}
      </div>
      <div className="grid grid-cols-1 @2xl:grid-cols-3 gap-4">
        {items.map((item, i) => (
          <div key={i} className={`reveal-fade-up reveal-d${Math.min(i + 2, 8)} bg-bg-2 border border-border-default rounded-xl p-5 transition-all hover:border-border-hover`}>
            <Quote size={20} className="text-green/30 mb-3" />
            <p className="text-[13px] text-text-1 leading-relaxed mb-4 italic">&ldquo;{item.quote}&rdquo;</p>
    ...
    <section className="px-6 @md:px-10 py-16 @md:py-20">
      ...
      <div className="reveal-fade-up reveal-d2 max-w-2xl mx-auto">
        <div className="text-center">
          ...
          <p className="text-lg text-text-0 leading-relaxed mb-4 italic">&ldquo;{item.quote}&rdquo;</p>
          {item.rating && (
          ...
          <div className="flex items-center justify-center gap-4 mt-6">
            <button onClick={() => goTo(safeIndex - 1)} className="w-8 h-8 rounded-full border border-border-default bg-bg-2 flex items-center justify-center text-text-2 hover:text-text-0 hover:border-border-hover transition-colors">
              <ChevronLeft size={16} />
            </button>
            <div className="flex gap-1.5">
              {items.map((_, i) => (
                <button key={i} onClick={() => goTo(i)} className={`w-2 h-2 rounded-full transition-colors ${i === safeIndex ? "bg-green" : "bg-bg-4 hover:bg-bg-5"}`} />
              ))}
            </div>
            <button onClick={() => goTo(safeIndex + 1)} className="w-8 h-8 rounded-full border border-border-default bg-bg-2 flex items-center justify-center text-text-2 hover:text-text-0 hover:border-border-hover transition-colors">
  ...
  return (
    <section className="px-6 @md:px-10 py-16 @md:py-24">
      <div className="max-w-2xl mx-auto text-center">
        <div className="reveal-fade-up reveal-d1">
          <Quote size={48} className="text-green/20 mx-auto mb-6" />
        </div>
        <p className="reveal-fade-up reveal-d2 text-xl @md:text-2xl text-text-0 leading-relaxed mb-8 italic font-display">&ldquo;{item.quote}&rdquo;</p>
        {item.rating && (
