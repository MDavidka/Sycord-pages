...
const defaultTiers: PricingTier[] = [
  { name: "Starter", price: "$0", period: "/month", description: "For personal projects", features: ["1 website", "5 blocks", "Basic export", "Community support"], cta: "Get Started" },
  { name: "Pro", price: "$19", period: "/month", description: "For professionals", features: ["Unlimited websites", "All blocks", "Custom domains", "Priority support", "Agent API access", "Version history"], cta: "Upgrade to Pro", featured: true },
  { name: "Team", price: "$49", period: "/month", description: "For teams and agencies", features: ["Everything in Pro", "Team collaboration", "Custom components", "SSO", "Dedicated support"], cta: "Contact Sales" },
]
        ...
        {tiers.map((tier, i) => (
          <div
            ...
            className={`reveal-fade-up reveal-d${Math.min(i + 2, 8)} relative rounded-xl p-6 flex flex-col transition-all ${
              tier.featured ? "bg-bg-2 border-2 border-green accent-glow-ring" : "bg-bg-2 border border-border-default hover:border-border-hover"
            }`}
          >
            {tier.featured && (
            ...
            <div className="mb-4">
              ...
              {tier.description && <p className="text-[11px] text-text-3">{tier.description}</p>}
            </div>
            <div className="mb-4">
              <span className="text-3xl font-bold tracking-tight">{tier.price}</span>
              {tier.period && <span className="text-text-3 text-sm">{tier.period}</span>}
            </div>
            ...
            <button
              className={`w-full py-2.5 rounded-lg text-sm font-semibold transition-all ${
                tier.featured ? "bg-green text-black hover:bg-green-dim hover:accent-glow-lg" : "bg-bg-3 text-text-0 border border-border-default hover:bg-bg-4 hover:border-border-hover"
              }`}
            >
              {tier.cta}
            </button>
              ...
              <tr key={i} className="border-b border-border-subtle">
                ...
                {tiers.map((tier, j) => (
                  <td key={j} className="py-2.5 px-3 text-center">
                    {tier.features.includes(feature) ? <Check size={14} className="text-green mx-auto" /> : <span className="text-text-3">-</span>}
                  </td>
                ))}
              </tr>
            ))}
