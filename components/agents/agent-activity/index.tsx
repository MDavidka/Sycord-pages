...
function getActiveLabel(type: AgentActivityContentType) {
  ...
  if (type === "tool") return "Running workspace tools…";
  if (type === "trace") return "Executing changes…";
  if (type === "mixed") return "Working through steps…";
  return "Thinking…";
}
...
function getSummary(
  ...
  if (type === "step" || type === "text") {
    return (
      <>
        Thought for <span className="tabular-nums font-semibold">{formatDuration(duration || 1)}</span>
      </>
    );
  }
  ...
  if (type === "trace") {
    return `Completed ${items.length} execution steps (${formatDuration(duration || 1)})`;
  }
  return `Thought and ran ${items.length} ${items.length === 1 ? "step" : "steps"}`;
}
...
export function AgentActivity({
  ...
  // When working, always stay expanded so the user sees real-time reasoning & tools!
  ...
  return (
    <div
      ...
      aria-busy={working}
      className={cn("w-full text-sm my-2", className)}
    >
      <div className="flex items-center justify-between">
        {working ? (
          <div
            id={triggerId}
            role="status"
            className="flex h-7 min-w-0 items-center gap-2 text-muted-foreground text-xs"
          >
            <Brain className="size-3.5 text-primary/70 animate-pulse" />
            {renderWorkingStatus
              ? renderWorkingStatus({ label: liveLabel, duration })
              : <ThinkingShimmer>{liveLabel}</ThinkingShimmer>}
            {duration > 0 ? (
              <span className="text-[11px] tabular-nums text-muted-foreground/60">({formatDuration(duration)})</span>
            ) : null}
          </div>
        ) : (
          <button
            ...
            onClick={toggle}
            className="group flex h-7 min-w-0 items-center gap-1.5 rounded-md px-1 text-left text-xs font-medium text-muted-foreground outline-none transition-colors hover:text-foreground focus-visible:ring-2 focus-visible:ring-ring"
          >
      ...
      <AgentDisclosure
        ...
        open={expanded}
        openHeight={expanded ? "auto" : 0}
      >
        <div
          ref={viewportRef}
          className={cn(
            "pr-1 transition-all",
            capped && expanded && !working ? "max-h-72 overflow-y-auto" : "max-h-96 overflow-y-auto",
          )}
        >
          <motion.div
            ref={contentRef}
            role="list"
            className={cn("space-y-1.5 py-1.5", contentClassName)}
          >
