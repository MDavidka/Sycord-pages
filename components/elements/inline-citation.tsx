...
export function InlineCitation({
  ...
  return (
    <p
      ...
      {...props}
    >
      {children ?? <>Optimistic updates keep the thread responsive while the server confirms the write</>}
      {sources[0] && (
        <Citation
          index={0}
          source={sources[0]}
      ...
      {children ? null : <>. The store already exposes a consistent snapshot for every subscriber</>}
      {sources[1] && (
        <Citation
          index={1}
          source={sources[1]}
      ...
      {children ? null : <>, so no extra reconciliation pass is needed.</>}
    </p>
  );
}
