...
function isAllowedPreviewUrl(raw: string): boolean {
  try {
    ...
    if ((hostname === "localhost" || hostname === "127.0.0.1") && process.env.NODE_ENV !== "production") return true
    return false
  } catch {
    return false
  }
...
function frameAncestorsForRequest(req: Request): string {
  ...
  ancestors.add("https://sycord.site")
  ancestors.add("https://www.sycord.site")
