  ...
  try {
    ...
    if (isAllowed) {
      if (parsed.protocol === "http:") {
        ...
        return false // Other http URLs are not allowed if allowed domains
      }
      return true
    }
    if (hostname === "localhost" || hostname === "127.0.0.1") {
...
function frameAncestorsForRequest(req: Request): string {
  ...
  // Hardcode allowed origins to prevent clickjacking SSRF from arbitrary dynamic request origins
