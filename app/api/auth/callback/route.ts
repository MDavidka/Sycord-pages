...
export async function GET(request: NextRequest) {
  try {
    ...
    if (tokenData.id_token) {
      ...
      console.warn("[CALLBACK] Step 6: Skipping Nonce validation (id_token missing)")
    }

    console.log("[CALLBACK] Step 7: Fetching Vercel user details...")
    const vercelUser = await fetchVercelUser(tokenData.access_token)
    console.log("[CALLBACK] Vercel User fetched:", vercelUser.id)

    console.log("[CALLBACK] Step 8: Persisting user to database...")
    await persistUserToDB(vercelUser, tokenData)
    ...
    console.log("[CALLBACK] Step 9: Cleanup done. Setting session...")

    const expiresAt = Date.now() + tokenData.expires_in * 1000
    cookieStore.set("vercel_auth_token", tokenData.access_token, {
      httpOnly: true,
      secure: process.env.NODE_ENV === "production",
      sameSite: "lax",
      maxAge: 60, // 1 minute - just enough for redirect
    })
    ...
    cookieStore.set("vercel_user_id", vercelUser.id, {
      httpOnly: true,
      secure: process.env.NODE_ENV === "production",
      sameSite: "lax",
      maxAge: 60,
    })
    cookieStore.set("vercel_user_email", vercelUser.email, {
      httpOnly: true,
      secure: process.env.NODE_ENV === "production",
      sameSite: "lax",
      maxAge: 60,
    })
    cookieStore.set("vercel_user_name", vercelUser.name || vercelUser.username, {
      httpOnly: true,
      secure: process.env.NODE_ENV === "production",
      sameSite: "lax",
      maxAge: 60,
    })
    cookieStore.set("vercel_expires_at", expiresAt.toString(), {
      httpOnly: true,
      secure: process.env.NODE_ENV === "production",
      sameSite: "lax",
      maxAge: 60,
    ...
    console.log("[CALLBACK] Step 10: Redirecting to callback handler...")
    return Response.redirect(new URL("/api/auth/callback-handler", request.url))
  } catch (error) {
...
async function persistUserToDB(vercelUser: any, tokenData: TokenData) {
  try {
    ...
    const updateData: any = {
      ...
      vercelRefreshToken: tokenData.refresh_token,
      vercelExpiresAt: Date.now() + tokenData.expires_in * 1000,
      provider: "vercel",
      updatedAt: new Date(),
    }

    if (tokenData.team_id) {
      updateData.vercelTeamId = tokenData.team_id
    }
