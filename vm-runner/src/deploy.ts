...
export type DeployResponse = {
  ...
  error?: string
  warning?: string
  debug?: DeploymentDebug
}

export type DeploymentDebug = {
  startedAt: string
  completedAt: string
  durationMs: number
  cwd: string
...
export async function deployProject(projectId: string, payload: DeployPayload, stream: DeployStreamWriter | null = null): Promise<DeployResponse> {
  ...
  for (let attempt = 0; attempt < MAX_START_RETRIES; attempt += 1) {
    ...
    if (combined.includes("eaddrinuse") || combined.includes("address already in use")) {
      ...
      if (!debug.portAllocation.blockedBy) debug.portAllocation.blockedBy = []
  ...
  const baseResponse = (success: boolean, error?: string): DeployResponse => {
