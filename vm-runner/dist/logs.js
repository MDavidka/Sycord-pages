...
export async function ensureProjectLogDir(projectId) {
    await fs.mkdir(getProjectLogsDir(projectId), { recursive: true });
}
function logPath(projectId, type) {
    return path.join(getProjectLogsDir(projectId), `${type}.log`);
}
export async function resetProjectLogs(projectId) {
    await ensureProjectLogDir(projectId);
    await Promise.all(["deploy", "build", "runtime", "error", "health"].map((type) => fs.writeFile(logPath(projectId, type), "")));
}
export async function appendLog(projectId, type, line) {
    await ensureProjectLogDir(projectId);
    await fs.appendFile(logPath(projectId, type), `${line}\n`);
}
export async function readLog(projectId, type, limit = 300) {
    try {
