...
export async function streamContinueAgent(options: {
  ...
  if (!projectId) {
    throw new Error('No project context for Continue agent');
  }

  let message = options.message;
  const skills = options.activeSkillIds?.length
    ? buildSkillsPrompt(options.activeSkillIds)
    : '';
  if (skills) {
    message = `${skills}\n\n---\n\n${message}`;
  }
  ...
  if (!res.body) {
    throw new Error('Agent stream missing body');
  }
