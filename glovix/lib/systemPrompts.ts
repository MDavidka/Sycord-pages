...
- **deploy** → \`POST /api/workspace/deploy\` — provisions the project's container/application on the **Dokploy** platform (created automatically on first deploy) and starts a deployment, returning the live URL (e.g. \`https://your-project.sycord.site\`).
