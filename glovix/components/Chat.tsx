    ...
    useEffect(() => {
        ...
        const timer = window.setTimeout(() => {
            void (async () => {
                ...
                try {
                    // Rebuild older actions from the durable Turso session even when the
                    // turn already completed — live `actions` are cleared after each run.
                    if (knownTursoId && toolsMissing) {
                        try {
                            const snapshot = await fetchAgentSessionSnapshot({
                                projectId,
                                tursoSessionId: knownTursoId,
                                sinceId: 0,
                            ...
                            if (restored.length > 0) {
                                ...
                                if (target?.role === 'assistant') {
                                    target.tool_calls = restored;
                                    if (!target.tursoSessionId) target.tursoSessionId = knownTursoId;
                                    state.setMessages([...messages]);
                                    if (currentChatId && user) {
                                ...
                                // Seed live feed so the UI shows restored tools immediately.
                                setActions(
                                    restored.map((tc, i) => ({
                                        id: tc.id || `restored_${i}`,
