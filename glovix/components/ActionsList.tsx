...
function FileChipStack({ paths }: { paths: string[] }) {
  ...
  return (
    <div className="flex min-w-0 flex-col gap-1">
      ...
      {overflow > 0 && (
        <span className="pl-[22px] font-mono text-[12px] text-white/35">…</span>
      )}
    </div>
  )
}
  ...
  const body = (
    ...
    >
      {hasDetails ? (
        <ChevronRight
          className={cn(
            'mt-0.5 size-4 shrink-0 text-white/40 transition-transform',
            open && 'rotate-90',
          )}
          strokeWidth={1.8}
        />
      ...
      <div className="min-w-0 flex-1">
        <div className="flex min-w-0 items-start gap-3">
          <span className="shrink-0 text-sm font-semibold text-white/85">{group.label}</span>
          ...
          {!isFileStack && !isCommandStack && (
            <span className="min-w-0 flex-1 truncate font-mono text-[13px] text-white/50">
...
function stackActions(actions: StreamingAction[]): StackedGroup[] {
  ...
  for (const action of actions) {
    ...
    const canMerge =
      ...
      stack !== 'solo' &&
      last.active === active &&
      // Keep mixed error/success stacks for finished file/command batches
      (active || last.actions.every(a => a.status !== 'error') || action.status === 'error' || true)
