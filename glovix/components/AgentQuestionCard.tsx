...
export function AgentQuestionCard({
  ...
  const confirmButton = (enabled: boolean, label: string, onClick: () => void) => (
    <Button
      ...
      onClick={onClick}
      className={cn(
        'mt-3 h-9 gap-2 rounded-xl px-4 text-[13px] font-medium',
        isDark
      ...
      {question.questionType === 'input' && (
        <div className="mt-3 space-y-2">
          <Input
            ...
            className={cn(
              'h-11 rounded-[10px] text-[14px]',
              isDark
                ? 'border-[#2a2b2e] bg-[#141516] text-white placeholder:text-[#6b6c6f] focus-visible:border-[#4A90E2]/60'
        ...
        <div className="mt-3 space-y-2">
          {options.length === 0 ? (
            ...
            >
              <SelectTrigger
                className={cn(
                  'h-11 w-full rounded-[10px] text-[14px]',
                  isDark
                    ? 'border-[#2a2b2e] bg-[#141516] text-white'
                    : 'border-gray-200 bg-gray-50 text-gray-900',
                )}
              >
              ...
              <SelectContent
                className={cn(
                  isDark ? 'border-[#2a2b2e] bg-[#1c1d1f] text-white' : undefined,
                )}
              >
                <SelectGroup>
                  {options.map((opt) => (
              ...
              {options.map((opt) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      'min-h-[44px] min-w-[44px] max-w-full rounded-[14px] px-3.5 py-2.5 text-left text-[13px] font-medium leading-snug transition-all active:scale-[0.97] disabled:opacity-50',
