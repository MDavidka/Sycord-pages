// shadcn/ui design system for Syra-generated sites.
//
// Every site Syra builds uses shadcn/ui (https://ui.shadcn.com) as its design
// system. Rather than ask the model to reproduce these primitives (slow and
// error-prone), Syra injects the canonical component source, the theme tokens
// (globals.css), the matching tailwind.config.js and the `cn` helper. The model
// then composes the UI by importing from "@/components/ui/*".
//
// All components target Tailwind CSS v3 + the CSS-variable theme below and are
// React 18 compatible (Radix UI primitives).

/** lucide-react names referenced by these components (kept for documentation). */
export const SHADCN_COMPONENTS = [
  "button", "card", "input", "label", "textarea", "badge", "separator", "avatar",
 ...
 * packages, cva, clsx and tailwind-merge are picked up automatically from the
 * component imports, but we list the essentials here too for safety.
 */
...
export const SHADCN_GLOBALS_CSS = `@tailwind base;
@tailwind components;
@tailwind utilities;
...
export function shadcnTailwindConfig(): string {
...
const DROPDOWN_MENU = `"use client"
...
/**
 * The full set of design-system files to inject (when missing). Keyed by repo
 * path. globals.css and tailwind.config.js are handled separately by the
 * scaffolder because they have their own existence checks.
 */
export function shadcnComponentFiles(): Record<string, string> {
  return {
    "lib/utils.ts": UTILS,
    ...
    "components/ui/sonner.tsx": SONNER,
