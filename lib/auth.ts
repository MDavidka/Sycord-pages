    ...
    // Custom Vercel Provider with Explicit Token Handling
    {
      ...
      token: {
        ...
        async request(context) {
            ...
            // Fallback to process.env if provider properties are missing (safety net)
            const clientId = provider.clientId || process.env.VERCEL_CLIENT_ID
            const clientSecret = provider.clientSecret || process.env.VERCEL_CLIENT_SECRET
            ...
            console.log("[v0-DEBUG] Token Request Prepared:", {
                code: params.code ? "PRESENT" : "MISSING",
                redirect_uri: redirectUri,
                client_id_source: provider.clientId ? "PROVIDER" : "ENV",
                client_id_val: clientId ? clientId.substring(0, 5) + "..." : "UNDEFINED",
                auth_method: "Body Params (client_secret_post)"
            });

            if (!clientId || !clientSecret) {
                throw new Error("Missing Vercel Client ID or Secret in token request");
            }

            // Vercel documentation says params in body.
            // https://vercel.com/docs/integrations/create-integration/step-2-oauth#exchange-code-for-access-token
            ...
            const body = new URLSearchParams({
                ...
                code: params.code!,
                redirect_uri: redirectUri,
                grant_type: "authorization_code", // Vercel docs don't strictly require this but it's standard
            });

            // Note: If this fails with invalid_client, ensure the CLIENT ID in env vars matches the one from Vercel Integration Console.
            // Also ensure the environment where this runs has the updated env vars.
